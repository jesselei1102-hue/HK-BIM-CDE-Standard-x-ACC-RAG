...
version: "2.0"
date: "2026-07-13"
disclaimer: "ACC Approval Workflow ≠ ISO 19650 Information Gateway 完整定義。Gateway 需 Workflow + 遷夾紀律 + 屬性更新共同實現。"
---
...
| 狀態變更（文件夾） | **人工遷夾**（DC 操作） | ⚠️ 人工 |
| 狀態變更（屬性） | **人工更新** Custom Attribute | ⚠️ 人工 |
| 自動門控 | ⚠️ ACC Workflow 完成後 **無自動遷夾/改屬性** | ❌ 缺口 |

> **核心聲明**：ACC Approval Workflow 是 Gateway 的 **審批環節**，但不是完整 Gateway。完整 Gateway = Workflow 通過 + DC 遷夾 + DC 更新屬性。三者缺一不可。
...
Published (A1–A6)
     ...
     ▼
┌─────────────────────────┐
│ Gateway 3: Archive      │  ACC: DC 手動移至 Archive
...
| 4 | Design Lead | 確認可共享 | 人工確認 |
| 5 | Document Controller | **手動複製**文件到 `02_Shared/{Discipline}/` | ⚠️ 人工遷夾 |
| 6 | Document Controller | 更新 Custom Attribute: `Status Code` = S1/S2 | ⚠️ 人工更新 |
| 7 | (Optional) System | Shared 文件夾 Auto-trigger 命名驗證 | ✅ 可自動 |

> ⚠️ 步驟 5-6 是 **人工紀律**。Workflow Approve 後 ACC 僅發通知，**不自動遷夾也不改屬性**。
...
| 4 | Client Rep | 客戶審批 | 人工 |
| 5 | Document Controller | **手動複製**到 `03_Published/{用途}/` | ⚠️ 人工遷夾 |
| 6 | Document Controller | 更新 `Status Code` = A1/A4/A6 等 | ⚠️ 人工更新 |
| 7 | Document Controller | 更新 `Approval Date` 屬性 | ⚠️ 人工 |
...
4. **完成後動作**
   - Notification: 通知提交人和 Document Controller
   - ⚠️ **注意**：ACC 無「完成後自動遷夾」選項——須通過通知觸發 DC 人工操作

5. **保存與激活**
   - Save as Draft → 測試（用測試文件跑一遍） → Activate

> **Product Help**: [Create and Edit Approval Workflows](https://help.autodesk.com/cloudhelp/ENU/Docs-Reviews/files/getting-started-reviews/Reviews_Create_Edit.html)
...
- 但它 **不是** ISO 19650 Gateway 的完整實現
- Gateway = Auto-trigger + 人工遷夾前的 Workflow + 屬性更新
- Auto-trigger 更適合做 **二次驗證**（如命名合規性 check）而非主審批流
...
| 8 | Document Controller 明確知道「Approve 後手動遷夾」的職責 | ☐ |
| 9 | Transmittal 模板已配置（編號規則、預設接收方） | ☐ |
| 10 | Activity Log 導出已測試 | ☐ |
...
| 事項 | 缺口說明 | BEP 章節建議 |
|------|----------|-------------|
| Gateway 完整定義 | ACC Workflow ≠ 完整 Gateway；BEP 須定義「Workflow 通過 + DC 遷夾 + 屬性更新」為一個原子操作 | §4 CDE Workflow |
| 遷夾時效 | Workflow Approve 後多久內 DC 須完成遷夾？建議 SLA: 1 工作日 | §4 CDE Workflow |
| 自動化方案（如有） | 若使用 ACC API/Webhook 自動遷夾，須記錄技術方案和回退機制 | §7 Technology |
| Reject 後重新提交規則 | 修改後是否需要重走全部步驟，還是從被拒步驟開始？ | §4 CDE Workflow |
| Timeout 處理 | 審核人超時不回應如何處理？Auto-reject / Escalate / 手動提醒？ | §4 CDE Workflow |
| 版本號遞增時機 | 何時 +1？遷出 WIP 時？還是進入 Published 時？ | §3 Naming |
