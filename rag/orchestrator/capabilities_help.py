...
需要单轨时可用：`--corpus docs` / `hk_cde` / `playbook`；默认 `hybrid`。更全的命令见项目里的 `COMMANDS.md`。
"""
...
單軌可用：`--corpus docs` / `hk_cde` / `playbook`；預設 `hybrid`。詳見 `COMMANDS.md`。
"""
...
Use `--corpus docs` / `hk_cde` / `playbook` for a single track; default is `hybrid`. See `COMMANDS.md` for all CLIs.
"""
